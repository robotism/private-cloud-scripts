...
install_k8s(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/kubernetes:v1.27.13  --masters ${masters:-""} -p ${password}
  fi
  if [ -n "$nodes" ]; then
...
isntall_helm(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/helm:v3.14.1
  fi
}
...
install_cilium(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/cilium:v1.15.5
...
install_openebs(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/openebs:v3.10.0
  fi
}
...
install_longhorn(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/longhorn:v1.6.1
  fi
}
...
install_cert_manager(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/cert-manager:v1.14.5
...
install_istio(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/istio:v1.20.1 \
...
install_metrics_server(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/metrics-server:v0.6.4
  fi
}
...
install_kube_state_metrics(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/kube-state-metrics:v2.4.2
  fi
}
...
install_ingress_nginx(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/ingress-nginx:v1.9.4 \
...
install_higress(){
  ...
  sudo sealos run ${image_registry}/${image_repository}/higress:v1.3.6 \
  -e HELM_OPTS=" \
